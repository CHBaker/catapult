...
def _ExecutableExtensions():
    ...
    # e.g. ['.com','.exe','.bat','.cmd']
    return [x[1:].upper() for x in exts]
    # e.g. ['COM','EXE','BAT','CMD']
...
def _AddDirToPythonPath(*path_parts):
    ...
    if os.path.isdir(path) and path not in sys.path:
        # Some callsite that use telemetry assumes
        # that sys.path[0] is the directory
...
def WaitFor(condition, timeout):
    ...
    while elapsed_time < timeout:
        ...
        poll_interval = min(
            max(elapsed_time / 10., MIN_POLL_INTERVAL_IN_SECONDS),
